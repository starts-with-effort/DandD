...
import type { Mesa, Cliente, MenuItem, Estado } from '@/lib/api';
import { getUserInfo } from '@/lib/auth'; // Importar función para obtener información del usuario
...
export default function CrearPedidoPage() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      // Si hay mesas disponibles, seleccionamos la primera por defecto
  ...
  const handleRemoveOrden = (temporalId: string) => {
    setFormData(prev => ({
      ...prev,
      ordenes: prev.ordenes.filter(orden => orden.temporal_id !== temporalId)
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const pedidoData = {
        ...
        usuario: userInfo.id // Añadir el ID del usuario actual
      };
      ...
      // 2. Crear las órdenes asociadas al pedido
      ...
      setTimeout(() => {
        router.push('/gerente/pedidos');
      }, 1500);
      
    } catch (error) {
      console.error('Error al crear el pedido:', error);
      setError('Ocurrió un error al crear el pedido. Por favor, inténtalo de nuevo.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const getMenuItemName = (id: string): string => {
    ...
    console.log("Available items:", menuItems);
    ...
    <ProtectedRoute allowedGroups={['Administrador', 'Gerente']}>
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-700">Crear Nuevo Pedido</h1>
          <button
            onClick={() => router.push('/gerente/pedidos')}
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {formData.ordenes.map((orden, index) => (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => handleRemoveOrden(orden.temporal_id!)}
